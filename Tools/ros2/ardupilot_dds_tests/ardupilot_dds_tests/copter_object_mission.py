...
class CopterLawnmower(Node):
    ...
    def handle_camel_event(self, altitude):
        ...
        camel_lat, camel_lon = self.camel_position
        self.get_logger().info(f"Diverting to camel at ({camel_lat}, {camel_lon})")
        
        camel_wp = self.create_waypoint(camel_lat, camel_lon, altitude)
        self.send_goal_position(camel_wp)
        
        if self.wait_for_waypoint(camel_wp):
            self.get_logger().info("Reached camel location. Loitering...")
            time.sleep(5)  # loiter 5 sec (replace with proper loiter command if available)
        else:
            self.get_logger().warn("Failed to reach camel location")
